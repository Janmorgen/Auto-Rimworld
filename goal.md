...
| **history read as state** | a tally of events reported where the current answer was wanted | `check30.sh` counting every "pen is closed" ever printed, while the game showed *Pen not enclosed*. Same script, twice more under training mode, because replaying time makes a cumulative counter lie loudly: three colonists showed 24 rooms by day 7 (two live, 22 the same few rebuilt across four replays), and one kidnapping showed as a death and a taken |
...
| **a circle where a route was wanted** | straight-line distance standing in for whether a thing can be reached, and in how long | `HostilesWithin` compared squared chord against a radius, so run 206 withdrew from ten infestation insects sealed in rock at `danger None`, stood down, withdrew again, four times an hour, `roomsEver: 0` at day 5. `TicksToReach` had the same flaw in gentler form — reachability real, distance a chord |
